...
const Layout: React.FC<LayoutProps> = ({ children, currentView, changeView }) => {
  ...
  return (
    <div className="min-h-screen bg-[#F4F6FA] flex flex-col md:flex-row font-['Tajawal'] print:bg-white print:block" dir="rtl">
      {/* Mobile Header */}
      <div className="md:hidden relative z-20 print:hidden mb-6">
        {/* Main curved header background */}
        <div className="absolute inset-x-0 top-0 h-[80px] bg-[#2A2A40] rounded-b-[32px] overflow-hidden shadow-sm">
        </div>

        {/* Header Content */}
        <div className="relative h-[80px] px-6 flex justify-center items-center">
          <button onClick={() => setSidebarOpen(!isSidebarOpen)} className="p-2 text-white/90 bg-white/10 rounded-xl border border-white/5 absolute right-6 hover:bg-white/20 hover:text-white transition-all active:scale-95">
